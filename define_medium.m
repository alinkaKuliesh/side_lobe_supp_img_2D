...
function medium = define_medium(kgrid, transducer, pulse, scatters, margin, speckle_flag)
    ...
    speckle_perc = 0.005; % 0.5% variation
...
switch scatters
    ...
    case 'resolution grid'
       ...
       if mod (transducer.pitch, 2) == 0           
           yv = (((transducer.num_active_elements - 1) / 2 + 5) * transducer.pitch + transducer.pitch / 2)...
               * kgrid.dy + yv;           
       else
           yv = (((transducer.num_active_elements - 1) / 2 + 5) * transducer.pitch + (transducer.pitch - 1) / 2)...
               * kgrid.dy + yv;      
       end   
end
...
switch shape
    case 'cube'
        ...
        for i = 1 : length(x_mask)
            ...
            maps.density(x_mask(i), y_mask(i), z_mask(k)) = 15 * rho_water;
        end
